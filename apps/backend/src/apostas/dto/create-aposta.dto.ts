...
import { CreateApostaDto as ICreateApostaDto } from '@repo/utils';

export class CreateApostaDto implements ICreateApostaDto {
  @IsNotEmpty({ message: 'O ID do jogo é obrigatório' })
  @IsUUID('all', { message: 'O ID do jogo deve ser um UUID válido' })
  jogoId: string;

  @IsOptional()
  @IsUUID('all', { message: 'O ID do palpite recomendado deve ser um UUID válido' })
  palpiteId?: string;
